...
<div className="flex items-start gap-2">
  ...
  <div>
    <h2 className="text-xl font-bold text-gray-900">Callable Lead ID </h2>
...
<div className="flex items-center border rounded-lg px-3 py-2 md:gap-2">
  ...
  />
      ...
      <div className="bg-white flex items-center justify-center rounded-lg mt-4">
        <div className="bg-white w-full rounded shadow-md overflow-x-auto">

                <div className="relative md:w-64 m-4">
            ...
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-3 text-center border border-white-500">
                  <input
                    type="checkbox"
                    checked={paginatedLeads.every((lead) => selectedLeads.includes(lead.id))}
                    onChange={(e) => {
              ...
              {paginatedLeads.map((row,i) => {
                ...
                return (
                  <tr key={row.id}>
                    ...
                    <td className={`p-3 text-black text-center ${cellStyle}`}>{row.id}</td>
                    <td className={`p-3 text-black text-center ${cellStyle}`}>{row.firstName}</td>
                    <td className={`p-3 text-black text-center ${cellStyle}`}>{row.lastName}</td>
                    <td className={`p-3 text-black text-center ${cellStyle}`}>{row.created}</td>
                    <td className={`p-3 text-black text-center ${cellStyle}`}>
                                         <button onClick={() => { setSelectedLead(row); setIsModalOpen(true); }}>
                      <Eye className="h-5 w-5 text-blue-600 hover:text-blue-800 p-1 bg-[#32ADE60F] rounded-full" />
                    </button>
                    </td>
                  </tr>
                );
  ...
  return Array.from({ length: end - start + 1 }, (_, idx) => {
    ...
    return (
      <button
        ...
        className={`px-2 py-1 rounded text-sm ${page === pageNum ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
      >
        {pageNum}
      </button>
    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md overflow-hidden p-4">
            <div className="bg-blue-50 flex justify-between items-center px-6 py-4 rounded-[10px]">
              <h2 className="text-lg font-semibold text-blue-900">Callable Lead ID ({selectedLead.id})</h2>
              <button
